...
public class TenantUseCase {
    ...
    public TenantUseCase(TenantJpaRepository tenantRepo, OutboxEventJpaRepository outboxRepo,
    ...
    public Tenant create(String name, String plan, String region) {
        ...
        publishOutbox("TENANT", id.toString(), "tenant.created", Map.of("name", name, "plan", plan, "region", region));
        ...
        auditLogger.log(TenantContext.getTenantId().orElse(null), TenantContext.getSubject(),
                ...
                "TENANT_CREATED", "tenant", id.toString(),
    ...
    @Transactional(readOnly = true)
    public Page<Tenant> search(Tenant.TenantStatus status, String plan, String region, String name, Pageable pageable) {
        return tenantRepo.search(status, plan, region, name, pageable).map(TenantEntity::toDomain);
    }

    @Transactional
    public Optional<Tenant> update(UUID id, String name, String plan, String region, Tenant.TenantStatus status) {
        return tenantRepo.findById(id).map(entity -> {
            ...
            publishOutbox("TENANT", id.toString(), "tenant.updated",
                    Map.of("name", entity.getName(), "plan", entity.getPlan()));
            auditLogger.log(TenantContext.getTenantId().orElse(null), TenantContext.getSubject(),
                    TenantContext.getRoles().toString(), TenantContext.getPerms().toString(),
                    "TENANT_UPDATED", "tenant", id.toString(),
    ...
    public boolean softDelete(UUID id) {
        return tenantRepo.findById(id).map(entity -> {
            ...
            auditLogger.log(TenantContext.getTenantId().orElse(null), TenantContext.getSubject(),
                    TenantContext.getRoles().toString(), TenantContext.getPerms().toString(),
                    "TENANT_DELETED", "tenant", id.toString(),
                    null, null, 204, TenantContext.getCorrelationId(), null);
            return true;
        }).orElse(false);
    }

    private void publishOutbox(String aggregateType, String aggregateId, String eventType, Map<String, String> data) {
