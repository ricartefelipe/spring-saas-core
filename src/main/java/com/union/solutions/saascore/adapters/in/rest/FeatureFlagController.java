...
public class FeatureFlagController {
    ...
    public ResponseEntity<?> create(@PathVariable UUID tenantId,
        ...
        FeatureFlagEntity flag = flagService.create(tenantId, request.name(), request.enabled(),
                request.rolloutPercent(), request.allowedRoles());
    ...
    public ResponseEntity<List<FlagDto>> list(@PathVariable UUID tenantId) {
        enforceTenantAccess(tenantId);
        List<FlagDto> flags = flagService.listByTenant(tenantId).stream()
                .map(FlagDto::from).toList();
        return ResponseEntity.ok(flags);
    }

    @PatchMapping("/{flagName}")
    public ResponseEntity<?> update(@PathVariable UUID tenantId, @PathVariable String flagName,
                                    @RequestBody UpdateFlagRequest request) {
        enforceTenantAccess(tenantId);
        return flagService.update(tenantId, flagName, request.enabled(),
                        request.rolloutPercent(), request.allowedRoles())
    ...
    public record CreateFlagRequest(@NotBlank String name, boolean enabled,
                                    int rolloutPercent, List<String> allowedRoles) {}
    public record UpdateFlagRequest(Boolean enabled, Integer rolloutPercent, List<String> allowedRoles) {}
    public record FlagDto(UUID id, UUID tenantId, String name, boolean enabled,
                          int rolloutPercent, String allowedRoles, Instant createdAt, Instant updatedAt) {
        public static FlagDto from(FeatureFlagEntity e) {
            return new FlagDto(e.getId(), e.getTenantId(), e.getName(), e.isEnabled(),
                    e.getRolloutPercent(), e.getAllowedRoles(), e.getCreatedAt(), e.getUpdatedAt());
        }
    }
}
