...
public class TenantSnapshotController {
    ...
    public TenantSnapshotController(TenantUseCase tenantUseCase, PolicyService policyService,
    ...
    public ResponseEntity<?> policies(@PathVariable UUID id) {
        return tenantUseCase.getById(id)
                .map(t -> {
                    List<PolicyEntity> policies = policyService.getApplicablePolicies(t.getPlan(), t.getRegion());
                    return ResponseEntity.ok(policies.stream().map(p -> Map.of(
                            "id", p.getId(),
        ...
        return tenantUseCase.getById(id)
                .map(t -> ResponseEntity.ok(
                        flagService.listByTenant(id).stream()
                                .map(FeatureFlagController.FlagDto::from).toList()))
                .orElse(ResponseEntity.notFound().build());
    }
}
