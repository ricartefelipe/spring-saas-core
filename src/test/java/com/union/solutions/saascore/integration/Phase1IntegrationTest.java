...
class Phase1IntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        adminToken = tokenIssuer.issue("admin@test", "00000000-0000-0000-0000-000000000099",
                List.of("admin"), List.of("tenants:read", "tenants:write", "policies:read", "policies:write", "flags:read", "flags:write"),
                "enterprise", "us-east-1");
        freeToken = tokenIssuer.issue("free@test", "00000000-0000-0000-0000-000000000099",
                List.of("user"), List.of("admin:write"), "free", "us-east-1");
    }
    ...
    void tenantCrud_worksEndToEnd() throws Exception {
        MvcResult createResult = mvc.perform(post("/v1/tenants")
                        .header("Authorization", "Bearer " + adminToken)
        ...
        mvc.perform(get("/v1/tenants/" + tenantId)
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk());
        ...
        mvc.perform(patch("/v1/tenants/" + tenantId)
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"Updated Corp\"}"))
                .andExpect(status().isOk());

        mvc.perform(delete("/v1/tenants/" + tenantId)
                        .header("Authorization", "Bearer " + adminToken))
    ...
    void policyCrud_worksEndToEnd() throws Exception {
        MvcResult createResult = mvc.perform(post("/v1/policies")
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"permissionCode\":\"test:action\",\"effect\":\"ALLOW\",\"allowedPlans\":[\"pro\"],\"allowedRegions\":[],\"enabled\":true,\"notes\":\"test\"}"))
                .andExpect(status().isCreated())
                .andReturn();
        JsonNode created = objectMapper.readTree(createResult.getResponse().getContentAsString());
        String policyId = created.get("id").asText();

        mvc.perform(get("/v1/policies/" + policyId)
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk());

        mvc.perform(delete("/v1/policies/" + policyId)
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isNoContent());
    }
    ...
    void abacDeny_isAudited() throws Exception {
        mvc.perform(post("/v1/policies")
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"permissionCode\":\"tenants:write\",\"effect\":\"DENY\",\"allowedPlans\":[\"free\"],\"allowedRegions\":[],\"enabled\":true,\"notes\":\"deny free\"}"))
                .andExpect(status().isCreated());

        mvc.perform(post("/v1/tenants")
                        .header("Authorization", "Bearer " + freeToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"Denied Corp\",\"plan\":\"free\",\"region\":\"us-east-1\"}"))
                .andExpect(status().isForbidden());

        MvcResult auditResult = mvc.perform(get("/v1/audit?action=ACCESS_DENIED")
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void devToken_endpointWorks() throws Exception {
        mvc.perform(post("/v1/dev/token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"sub\":\"dev@test\",\"tid\":\"00000000-0000-0000-0000-000000000001\",\"roles\":[\"admin\"],\"perms\":[\"test:read\"],\"plan\":\"pro\",\"region\":\"us-east-1\"}"))
                .andExpect(status().isOk())
    ...
    void correlationId_isReturnedInHeader() throws Exception {
        mvc.perform(get("/healthz")
                        .header("X-Correlation-Id", "my-corr-123"))
                .andExpect(status().isOk())
